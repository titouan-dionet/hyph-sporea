...
class StitcherGUI:
    ...
    def create_widgets(self):
        ...
        # Chemin de sortie
        ttk.Label(file_frame, text="Fichier de sortie:").grid(row=2, column=0, sticky=tk.W, pady=2)
        ttk.Entry(file_frame, textvariable=self.output_path, width=50).grid(row=2, column=1, sticky=tk.W+tk.E, pady=2)
        ttk.Button(file_frame, text="Parcourir", command=self.browse_output_path).grid(row=2, column=2, padx=5, pady=2)
    ...
    def browse_output_path(self):
        """Ouvre un dialogue pour sélectionner le fichier de sortie"""
    ...
    def update_output_path(self):
        """Met à jour automatiquement le chemin de sortie en fonction du dossier d'entrée"""
        ...
        output_path = os.path.join(output_dir, f"{sample_name}_assemblee.jpeg")
    ...
    def run_overlap_detection(self):
        ...
        def detection_thread():
            try:
                ...
                if not files:
                    self.log(f"Erreur: Aucun fichier {file_ext} trouvé dans le dossier.")
                ...
                output_file = os.path.join(input_dir, f"{sample_name}_overlap.json")
                ...
                self.set_status("Détection terminée")
    ...
    def run_stitching(self):
        ...
        def stitching_thread():
            try:
                ...
                self.log(f"Fichier de sortie: {self.output_path.get()}")
                ...
                if self.auto_overlap.get():
                    ...
                    self.log(f"Chevauchement détecté: {h_overlap}x{v_overlap} pixels")
                ...
                # Lire les informations de l'image assemblée
                info_path = os.path.splitext(self.output_path.get())[0] + '.json'
                ...
                self.set_status("Assemblage terminé")
                self.set_progress(100)
